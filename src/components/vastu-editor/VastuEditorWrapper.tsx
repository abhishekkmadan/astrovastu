...
export function VastuEditorWrapper({
  ...
  const handleSaveMarker = useCallback(
    async (remedy: string) => {
      ...
      try {
        ...
        if (!demoMode && supabase) {
          ...
          if (error) {
            alert(`Could not verify your session: ${error.message}`);
            return;
          }
          if (!user) {
            alert("Your session has expired. Please sign in again before saving markers.");
    ...
    [pendingMarker, layout.id, demoMode, supabase, router]
  );
