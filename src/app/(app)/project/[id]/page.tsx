...
export default async function ProjectDetailPage({
  ...
  const pdfLayout = safeLayouts.find((layout) => layout.workspace_phase === "full");
  const disabledPdfTitle =
    safeLayouts.length === 0
      ? "Upload a floor plan before generating a PDF"
      : "Complete layout setup before generating a PDF";
      ...
      <div className="mt-4 flex items-start justify-between">
        ...
        {pdfLayout ? (
          ...
          <button
            type="button"
            disabled
            title={disabledPdfTitle}
            className="flex cursor-not-allowed items-center gap-2 rounded-lg bg-surface-dim px-4 py-2 text-sm font-medium text-text-muted"
          >
            <FileText size={16} />
            Generate PDF
          </button>
